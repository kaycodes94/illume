...
<head>
    ...
    <style>
        ...
        .status-pill { font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.15em; padding: 4px 12px; border-radius: 999px; }
    </style>
</head>
<body class="antialiased font-sans min-h-screen pb-20">
    ...
    <nav class="border-b border-brand-border px-8 py-4 flex justify-between items-center sticky top-0 z-50 bg-white/80 backdrop-blur-md">
        ...
        <div class="flex items-center gap-6">
            <a href="founder_dash.php" class="text-[10px] uppercase tracking-widest text-gray-500 hover:text-brand-gold transition-colors font-bold">Dashboard</a>
    ...
    <main class="max-w-7xl mx-auto p-8 lg:p-12">
        <header class="mb-12 flex justify-between items-end">
            <div>
                ...
                <p class="text-xs text-gray-400 uppercase tracking-[0.2em] font-medium">Strategic Commission Tracking</p>
            </div>
            <div class="bg-brand-black text-white px-6 py-2.5 rounded-full text-[10px] uppercase tracking-widest font-bold">
            ...
            <?php else: ?>
                ...
                <div class="card p-8 rounded-[40px] hover:shadow-xl transition-all group">
                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                        <!-- Identity -->
                        <div class="flex items-center gap-6">
                            <div class="w-16 h-16 rounded-3xl bg-gray-50 flex items-center justify-center border border-brand-border">
                                <span class="font-serif italic text-xl text-brand-gold"><?= substr($o['order_number'], 0, 1) ?></span>
                            </div>
                            <div>
                                <p class="text-[10px] uppercase tracking-widest text-brand-gold font-bold mb-1"><?= htmlspecialchars($o['order_number']) ?></p>
                                <h3 class="text-xl font-serif text-brand-black mb-1"><?= htmlspecialchars($o['client_name']) ?></h3>
                                <p class="text-xs text-gray-400 uppercase tracking-widest"><?= htmlspecialchars($o['service_type']) ?></p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col lg:flex-row gap-12">
                            <div>
                                <p class="text-[9px] uppercase tracking-widest text-gray-400 mb-2 font-bold">Commission Value</p>
                        ...
                        <div class="flex items-center gap-8">
                            ...
                            <button onclick="alert('Viewing production specs...')" class="w-12 h-12 rounded-2xl border border-brand-border flex items-center justify-center text-gray-400 hover:text-brand-black hover:border-brand-black transition-all">
